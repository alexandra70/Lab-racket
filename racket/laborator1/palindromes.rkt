...
(define (num->base n b)
  (if (zero? n)
     '() ;if n == 0, then the result is null list, otherwise
...
(define (deep-rev L)
  (cond
    ((null? L) '())
    ((list? (car L)) (append (deep-rev (cdr L)) (list (deep-rev (car L))))) ;;dc elem curent e lista atunic e bai
    ((append (deep-rev (cdr L)) (list ( car L)))) ;;dc nu e list un reverse simplu doar apendez
    )
  )
...
(define (palindrome-list? L)
  (if (equal? (deep-rev L) (rev L)) ;;deci lista inversata simplu == deep-revul
      #t
      #f
      ))
...
(define (all-palindromes? n Bases)
  (if (null? Bases)
      #t
      (if (equal? (rev (num->base n (car Bases))) (num->base n (car Bases)))
...
(define baze (list 2 3 4 5 6 7 8 9 10))

(define (helper start baze l)
  (if (null? baze)
      l
      (if (palindrome? (num->base start (car baze)))
          (helper start (cdr baze) (append l '(1)))
          (helper start (cdr baze) l))
      )
  )
          ;;altfel vad dc in restul listelor am o baza care sa imi verifice conditia


(define (first-b-pal start b)
      (if (>= (length (helper start baze '())) b)
          start
...
;;trebuie sa iau lista lui de cifre si sa vad unde gasesc cea mai lunga secventa
(define (verific lista cate_iau)
  (if (zero? cate_iau)
      '()
      (if (palindrome? lista)
          lista
          (verific (take lista cate_iau) (- cate_iau 1))
          )
      )
  )

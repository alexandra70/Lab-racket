...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((seg1 (get-line-segment))
         (start (get-start-point seg1))
         (end (get-end-point seg1))
         (x1 (car start))
         (y1 (cdr start))
         (x2 (car end))
         (y2 (cdr end)))
    (sqrt (+ (* (abs (- x2 x1)) (abs (- x2 x1))) (*(abs (- y2 y1)) (abs (- y2 y1)))))))
...
(define (distance x y)
  ;;(compute-length get-line-segment get-start-point get-end-point)
  (compute-length (λ () (cons x y)) (λ (start) x) (λ (end) y)))
...
(define (compute-f-with-step2 f a b step)
  (let iter ((i b) (result '()))
    (if (> a i)
        result
        (iter (- i step) (cons (f i) result)))))

(define (compute-f-with-step f a b step)
  (let iter ((i a) (result '()))
    (if (> i b)
        result
        (cons (f i) (iter (+ i step) result)))))
...
(define (num-concat x y)
  (string->number (let (( +  (λ(n m) (string-append n m)))
        (x (number->string x))
        (y (number->string y)))
...
(define (compute-perimeter points)
  (let iter ((puncte (reverse (cons (car points) (reverse points)))) ;;construiesc puncte, mai am neoie inca o data de o pereche 
             (p 0))
    (if (null? (cdr puncte))
        p
        (iter (cdr puncte) (+ (distance (car puncte) (cadr puncte)) p))
        )
    )
  )

(define (compute-perimeterVERFICARI points)
  (let iter ((puncte (reverse (cons (car points) (reverse points)))) ;;construiesc puncte, mai am neoie inca o data de o pereche 
        )
    (if (null? puncte)
        0
...
(define (compute-perimeterNUmerg points)
  (let
      ((retin (car points))) ;????
      (let iter ((puncte points) ;;construiesc puncte, mai am neoie inca o data de o pereche 
             (p 0))
        (if (null? (cdr puncte))
            ( + p (distance (car points) retin))
            (iter (cdr puncte) (+ p (distance (car puncte) (cadr puncte)) ))
        )
    )))
...
(define (sumlist l acc)
  (if (null? l)
      acc
      (sumlist (cdr l) (+ (car l) acc)))) ;;(apply + l)
(define (3-sequence-maxB numbers separator)
...
(define (3-sequence-max numbers separator)
  (let*-values
    (((first rest) (splitf-at numbers (λ(x) (not (equal? x separator)))))
    ((second middle) (splitf-at (cdr rest) (λ(x) (not (equal? x separator)))))
    ((third end) (splitf-at (cdr middle) (λ(x) (not (equal? x separator))))))
    (max (sumlist first 0)  (sumlist second 0) (sumlist third 0))))

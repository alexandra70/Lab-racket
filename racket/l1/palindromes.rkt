...
;aici vreau sa scap de iecare data de primulelement
;pana cand L devine null, deci rev se realizeaza prin add head la sf lista
...
(define (palindrome-list? L) ;este deep la fel ca un reverse simplu?
...
(define (all-palindromes? n Bases)
  (if (null? Bases)
      #t ;;este palindrom un nr intr o baza, dc este caut sa vad dc e in toate, altfel stop
...
(define (palindromes-to-n n Bases)
  (if(< n 0)
     ...
     (if(all-palindromes? n Bases)
        (append(palindromes-to-n (sub n) Bases) (list n))
        ;;else continue to search
        (palindromes-to-n (sub n) Bases))))

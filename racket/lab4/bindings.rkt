...
(define (compute-perimeter points)
  (let ((start (car points)))
    (let iter ((puncte points) (result 0))
      (if (null? (cdr puncte))
          (+ result (distance start (car puncte)))
          (iter (cdr puncte) (+ result (distance (car puncte) (car (cdr puncte)))))))))
...
(define (player-wins? candies)
  (letrec
      ((player1 (λ (cand)
                  (cond
                    ((<= cand 0) #f)
                    ((> cand 0) (or (player2 (- cand 1)) (player2 (- cand 2)))))))

      (player2 (λ (cand)
                  (cond
                    ((<= cand 1) #t)
                    ((> cand 0) (or (player1 (- cand 2)) (player1 (- cand 3))))))))
